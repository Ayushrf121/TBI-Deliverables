        ...
        <div className="md:col-span-7 w-full bg-slate-900/50 border border-slate-800/80 rounded-2xl p-6 shadow-2xl backdrop-blur-xl space-y-6">
          <div className="flex items-center justify-between border-b border-slate-800/80 pb-4">
            <div>
              ...
              <p className="text-xs text-slate-400">Real-time state mutations engine</p>
            </div>
            <span className="text-[10px] font-mono font-bold px-2 py-1 rounded bg-slate-800 border border-slate-700 text-slate-400 tracking-tight">
              v1.2.0-STABLE
            </span>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all shadow-inner"
              />
          ...
          <form onSubmit={handleCreateTask} className="space-y-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all shadow-inner"
              />
              <button 
                type="submit" 
          ...
          <div className="space-y-3 pt-2">
            ...
            {loading ? (
              ...
              <div className="text-center py-10 px-4 rounded-xl bg-slate-950/40 border border-dashed border-slate-800">
                <p className="text-sm text-slate-500 italic">No document clusters captured in current pipeline.</p>
              </div>
            ) : (
              <ul className="space-y-2.5 max-h-[260px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-slate-800">
                {tasks.map((item) => {
                  const id = item._id || item.id;
                  return (
                    <li 
                      key={id} 
                      className="group flex items-center justify-between p-3 bg-slate-950/60 hover:bg-slate-950 border border-slate-850 rounded-xl shadow-sm transition-all duration-200 hover:border-slate-700"
                    >
                      ...
                      <button 
                        ...
                        className="text-xs font-semibold text-slate-500 hover:text-red-400 px-2.5 py-1.5 rounded-lg hover:bg-red-500/10 md:opacity-0 group-hover:opacity-100 transition-all duration-200"
                      >
                        Delete
                      </button>
                    </li>
